# OneBotSdk.Net.V10.EventHandlerExample

> **警告 / Warning**：当前附件中的 NapCat 服务器配置是 OneBot 11，不是 OneBot 10。请不要用它直接运行本示例；只有准备好符合规范的 OneBot 10 实现端后才能连接。

该示例使用标准 `EventHandler` 监听 OneBot 10 私聊、群、讨论组、通知、请求及元事件，并为每个事件输出 `RawJson`。消息链通过 LINQ `OfType` 筛选具体消息段。示例只接收事件，不发送消息。
...
```powershell
dotnet build samples/OneBotSdk.Net.V10.EventHandlerExample/OneBotSdk.Net.V10.EventHandlerExample.csproj -c Release -f net10.0
```
