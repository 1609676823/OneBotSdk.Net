...
/// <summary>
/// Identifies the normalized status of a OneBot 11 action response.
/// 标识 OneBot 11 动作响应的规范化状态。
/// </summary>
public enum OneBot11ResponseStatus
{
    /// <summary>
...
/// <summary>
/// Contains the common, field-tolerant portion of a OneBot 11 response envelope.
/// 包含 OneBot 11 响应信封中按字段容错解析的公共部分。
/// </summary>
public abstract class OneBot11ResponseBase : OneBot11JsonModel
{
    private protected OneBot11ResponseBase(
        JsonObject? source,
        OneBot11ActionTransportResult? transportResult)
    {
    ...
    /// </summary>
    [JsonIgnore]
    public OneBot11ResponseStatus StatusKind
    {
        get
        {
            if (string.Equals(Status, "ok", StringComparison.OrdinalIgnoreCase))
            {
                return OneBot11ResponseStatus.Ok;
            }

            if (string.Equals(Status, "async", StringComparison.OrdinalIgnoreCase))
            {
                return OneBot11ResponseStatus.Async;
            }

            if (string.Equals(Status, "failed", StringComparison.OrdinalIgnoreCase))
            {
                return OneBot11ResponseStatus.Failed;
            }

            return OneBot11ResponseStatus.Unknown;
        }
    }
    ...
    /// </summary>
    [JsonIgnore]
    public bool IsSuccess => StatusKind == OneBot11ResponseStatus.Ok && RetCode == 0;
    ...
    /// </summary>
    [JsonIgnore]
    public bool IsAsync => StatusKind == OneBot11ResponseStatus.Async && RetCode == 1;
}

/// <summary>
/// Represents a OneBot 11 response whose data is intentionally kept as JSON.
/// 表示将 data 原样保留为 JSON 的 OneBot 11 响应。
/// </summary>
public sealed class OneBot11Response : OneBot11ResponseBase
{
    private OneBot11Response(
        JsonObject? source,
        OneBot11ActionTransportResult? transportResult)
    ...
    /// </summary>
    public static OneBot11Response Parse(JsonObject? source)
    {
        return new OneBot11Response(source, null);
    }

    internal static OneBot11Response Parse(OneBot11ActionTransportResult transportResult)
    {
        if (transportResult == null)
        {
        ...
        return new OneBot11Response(transportResult.Response, transportResult);
    }
}

/// <summary>
/// Represents a OneBot 11 response with field-tolerant strongly typed data.
/// 表示包含按字段容错强类型 data 的 OneBot 11 响应。
/// </summary>
/// <typeparam name="TData">The action-specific response data type. / 动作专属响应数据类型。</typeparam>
public sealed class OneBot11Response<TData> : OneBot11ResponseBase
{
    private OneBot11Response(
        JsonObject? source,
    ...
    internal static OneBot11Response<TData> Parse(JsonObject? source, Func<JsonNode?, TData?> dataParser)
    {
        if (dataParser == null)
        {
        ...
        return new OneBot11Response<TData>(source, dataParser, null);
    }

    internal static OneBot11Response<TData> Parse(
        OneBot11ActionTransportResult transportResult,
        Func<JsonNode?, TData?> dataParser)
    {
        if (transportResult == null)
        ...
        return new OneBot11Response<TData>(transportResult.Response, dataParser, transportResult);
    }
}
