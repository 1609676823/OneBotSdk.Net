...
/// </summary>
internal sealed class OneBot11EventStream<TEvent> : IObservable<TEvent>
{
    ...
    {
        private readonly object _gate = new object();
        private OneBot11EventStream<TEvent>? _owner;
        private readonly IObserver<TEvent> _observer;

        internal Subscription(OneBot11EventStream<TEvent> owner, IObserver<TEvent> observer)
        {
            _owner = owner;
            _observer = observer;
        }
        ...
        public void Dispose()
        {
            OneBot11EventStream<TEvent>? owner;
            lock (_gate)
            {
                owner = _owner;
                _owner = null;
