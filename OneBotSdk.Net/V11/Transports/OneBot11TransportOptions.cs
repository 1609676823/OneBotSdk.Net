...
namespace OneBotSdk.Net.V11.Transports;

/// <summary>
/// Configures OneBot action calls made over HTTP.
/// 配置通过 HTTP 发起的 OneBot 动作调用。
/// </summary>
public sealed class OneBot11HttpActionTransportOptions
{
    /// <summary>
    /// Initializes HTTP action transport options.
    /// 初始化 HTTP 动作传输选项。
    /// </summary>
    public OneBot11HttpActionTransportOptions(Uri baseUri)
    {
    ...
    internal OneBot11HttpActionTransportOptions Snapshot()
    {
        ...
        return new OneBot11HttpActionTransportOptions(baseUri)
        {
...
/// </summary>
public sealed class OneBot11WebSocketTransportOptions
{
    /// <summary>
    ...
    internal OneBot11WebSocketTransportOptions Snapshot()
    {
        var receiveBufferSize = ReceiveBufferSize;
        var maxMessageBytes = MaxMessageBytes;
        var keepAliveInterval = KeepAliveInterval;
        OneBot11HttpActionTransportOptions.ValidatePositive(receiveBufferSize, nameof(ReceiveBufferSize));
        OneBot11HttpActionTransportOptions.ValidatePositive(maxMessageBytes, nameof(MaxMessageBytes));
        ...
        return new OneBot11WebSocketTransportOptions
        {
...
/// </summary>
public sealed class OneBot11ForwardWebSocketClientOptions
{
    /// <summary>
    /// Initializes forward WebSocket client options.
    /// 初始化正向 WebSocket 客户端选项。
    /// </summary>
    public OneBot11ForwardWebSocketClientOptions(Uri endpoint)
    {
    ...
    internal OneBot11ForwardWebSocketClientOptions Snapshot()
    {
        var endpoint = Endpoint ?? throw new ArgumentNullException(nameof(Endpoint));
        var accessToken = OneBot11HttpActionTransportOptions.NormalizeAccessToken(AccessToken, nameof(AccessToken));
        ValidateAbsoluteWebSocketUri(endpoint, nameof(Endpoint));
        var session = Session.Snapshot();
        var copy = new OneBot11ForwardWebSocketClientOptions(endpoint)
        {
            AccessToken = accessToken
        };
...
/// 配置反向 HTTP POST 事件接入。
/// </summary>
public sealed class OneBot11HttpPostEventIngressOptions
{
    /// <summary>
    ...
    internal OneBot11HttpPostEventIngressOptions Snapshot()
    {
        OneBot11HttpActionTransportOptions.ValidatePositive(MaxRequestBodyBytes, nameof(MaxRequestBodyBytes));
        return new OneBot11HttpPostEventIngressOptions
        {
            Secret = Secret,
