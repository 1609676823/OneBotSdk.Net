...
/// 标识操作失败时所在的传输边界。
/// </summary>
public enum OneBot11TransportError
{
    /// <summary>
...
/// </summary>
public sealed class OneBot11TransportException : Exception
{
    /// <summary>
    /// Initializes a transport exception.
    /// 初始化传输异常。
    /// </summary>
    public OneBot11TransportException(OneBot11TransportError error, string message)
        : base(message)
    {
        Error = error;
    }
    ...
    /// 使用底层失败信息初始化传输异常。
    /// </summary>
    public OneBot11TransportException(OneBot11TransportError error, string message, Exception innerException)
    ...
    /// 获取稳定的传输错误类别。
    /// </summary>
    public OneBot11TransportError Error { get; }
