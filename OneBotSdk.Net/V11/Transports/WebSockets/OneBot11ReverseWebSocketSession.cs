...
/// </summary>
public sealed class OneBot11ReverseWebSocketSession : IOneBot11ActionTransport, IDisposable
{
    private readonly OneBot11WebSocketSession _session;
    ...
    /// Optional bearer token that must match the handshake Authorization header.
    /// 必须与握手 Authorization 请求头匹配的可选 Bearer 令牌。
    /// </param>
    public OneBot11ReverseWebSocketSession(
        WebSocket acceptedWebSocket,
        OneBot11ReverseWebSocketMetadata metadata,
        ...
        string? expectedAccessToken = null)
    {
        ...
        if (expectedAccessToken != null && expectedAccessToken.Length != 0 && !metadata.HasBearerToken(expectedAccessToken))
        {
            throw new OneBot11TransportException(
                OneBot11TransportError.AuthenticationFailed,
                "The reverse WebSocket Authorization bearer token is missing or invalid.");
        }

        _session = new OneBot11WebSocketSession(acceptedWebSocket, dispatcher, options);
    }
    ...
    /// 获取从已接受握手中保存的元数据。
    /// </summary>
    public OneBot11ReverseWebSocketMetadata Metadata { get; }
    ...
    public bool CanSendActions =>
        Metadata.Role == OneBot11ReverseWebSocketRole.Universal ||
        Metadata.Role == OneBot11ReverseWebSocketRole.Api;
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            throw new OneBot11TransportException(
                OneBot11TransportError.InvalidConfiguration,
