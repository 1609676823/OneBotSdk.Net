...
/// <summary>
/// Opens and owns a forward WebSocket connection to a OneBot 11 implementation.
/// 建立并拥有到 OneBot 11 实现端的正向 WebSocket 连接。
/// </summary>
public sealed class OneBot11ForwardWebSocketClient : IOneBot11ActionTransport, IDisposable
{
    private readonly OneBot11ForwardWebSocketClientOptions _options;
    private readonly OneBot11EventDispatcher _dispatcher;
    private readonly object _stateGate = new object();

    private ClientWebSocket? _client;
    private OneBot11WebSocketSession? _session;
    ...
    /// 初始化正向 WebSocket 客户端。
    /// </summary>
    public OneBot11ForwardWebSocketClient(
        OneBot11ForwardWebSocketClientOptions options,
    ...
    /// </summary>
    public OneBot11WebSocketSession? Session
    {
        get
        {
            lock (_stateGate)
    ...
    public async Task ConnectAsync(CancellationToken cancellationToken)
    {
        ...
        var session = new OneBot11WebSocketSession(client, _dispatcher, _options.Session);
        ...
        lock (_stateGate)
        {
            ...
            {
                session.Dispose();
                throw new ObjectDisposedException(nameof(OneBot11ForwardWebSocketClient));
            }
    ...
    {
        ThrowIfDisposed();
        OneBot11WebSocketSession? session;
        lock (_stateGate)
        {
            session = _session;
        }
    ...
    /// <summary>
    /// Sends a normal close frame. Await <see cref="OneBot11WebSocketSession.Completion"/> when the peer acknowledgement is required.
    /// 发送正常关闭帧；如需等待对端确认，请等待 <see cref="OneBot11WebSocketSession.Completion"/>。
    /// </summary>
    ...
    private void HandleFaulted(OneBot11TransportException exception)
    {
        ...
        foreach (Action<OneBot11TransportException> handler in handlers.GetInvocationList())
        {
            try
            {
                handler(exception);
