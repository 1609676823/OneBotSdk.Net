...
/// </summary>
public sealed class OneBot10HttpActionTransport : IOneBot10ActionTransport, IDisposable
{
    ...
    /// </summary>
    public OneBot10HttpActionTransport(OneBot10HttpActionTransportOptions options)
        : this(options, null)
    {
    }
    ...
    /// 注入的客户端永远不会由此传输释放。
    /// </remarks>
    public OneBot10HttpActionTransport(OneBot10HttpActionTransportOptions options, HttpClient? httpClient)
    {
        if (options == null)
        {
        ...
        CancellationToken cancellationToken)
    {
        ...
        _ = echo;
        var detachedParameters = OneBot10TransportPayload.Clone(parameters) as JsonObject ?? new JsonObject();
        var body = OneBot10Json.Serialize(detachedParameters);
            ...
            using (response)
            {
                ...
                if (!response.IsSuccessStatusCode)
                {
                    var failure = ForAction(
                        OneBot10TransportError.HttpFailure,
                        action,
                ...
                try
                {
                    responseJson = OneBot10TransportPayload.DecodeUtf8(responseBody);
                }
                catch (OneBot10TransportException exception)
                {
                    exception.Action = action;
                ...
                try
                {
                    responseObject = OneBot10TransportPayload.ParseObject(responseJson);
                }
                catch (OneBot10TransportException exception)
                {
                    exception.Action = action;
    ...
    private async Task<byte[]> ReadResponseBodyAsync(HttpResponseMessage response, CancellationToken cancellationToken)
    {
        ...
        using (stream)
        {
            return await OneBot10TransportPayload
    ...
    private static OneBot10TransportException ForAction(
        OneBot10TransportError error,
        string action,
        string message,
        Exception? innerException = null)
    {
        var exception = innerException == null
            ? new OneBot10TransportException(error, message)
            : new OneBot10TransportException(error, message, innerException);
        exception.Action = action;
        return exception;
    }

    private static OneBot10TransportException AttachTrace(
    ...
    private static string? TryDecodeUtf8(byte[] payload)
    {
        try
        {
            return OneBot10TransportPayload.DecodeUtf8(payload);
        }
    ...
    private void ThrowIfDisposed()
    {
        ...
        {
            throw new ObjectDisposedException(nameof(OneBot10HttpActionTransport));
        }
    }
}
