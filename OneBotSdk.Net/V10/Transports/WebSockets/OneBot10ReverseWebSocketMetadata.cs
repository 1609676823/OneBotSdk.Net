...
/// <summary>
/// Describes the role declared by a OneBot 10 reverse WebSocket connection.
/// 描述 OneBot 10 反向 WebSocket 连接声明的角色。
/// </summary>
public enum OneBot10ReverseWebSocketRole
{
    /// <summary>
...
/// </summary>
public sealed class OneBot10ReverseWebSocketMetadata
{
    /// <summary>
    ...
    private OneBot10ReverseWebSocketMetadata(Dictionary<string, string> headers)
    {
    ...
    /// 获取规范化后的连接角色。
    /// </summary>
    public OneBot10ReverseWebSocketRole Role { get; }
    ...
    /// </summary>
    public static OneBot10ReverseWebSocketMetadata FromHeaders(IEnumerable<KeyValuePair<string, string>> headers)
    {
        if (headers == null)
        {
        ...
        return new OneBot10ReverseWebSocketMetadata(copy);
    }

    /// <summary>
    /// Verifies the legacy <c>Token</c> authorization scheme required by OneBot 10 reverse WebSocket.
    /// 验证 OneBot 10 反向 WebSocket 要求的旧版 <c>Token</c> 认证方案。
    /// </summary>
    public bool HasToken(string expectedAccessToken)
    {
        ...
        var supplied = Encoding.UTF8.GetBytes(Authorization.Substring(scheme.Length));
        return OneBot10HttpPostSignature.FixedTimeEquals(expected, supplied);
    }

    private static OneBot10ReverseWebSocketRole ParseRole(string? value)
    {
        if (string.Equals(value, "Universal", StringComparison.OrdinalIgnoreCase))
        {
            return OneBot10ReverseWebSocketRole.Universal;
        }

        if (string.Equals(value, "API", StringComparison.OrdinalIgnoreCase))
        {
            return OneBot10ReverseWebSocketRole.Api;
        }

        if (string.Equals(value, "Event", StringComparison.OrdinalIgnoreCase))
        {
            return OneBot10ReverseWebSocketRole.Event;
        }

        return OneBot10ReverseWebSocketRole.Unknown;
    }
