...
/// </summary>
public sealed class OneBot10WebSocketSession : IOneBot10ActionTransport, IDisposable
{
    private readonly WebSocket _webSocket;
    private readonly OneBot10EventDispatcher _dispatcher;
    private readonly OneBot10WebSocketTransportOptions _options;
    ...
    /// </summary>
    public OneBot10WebSocketSession(
        WebSocket webSocket,
        ...
        OneBot10WebSocketTransportOptions? options = null)
    {
        ...
        _options = (options ?? new OneBot10WebSocketTransportOptions()).Snapshot();
    }
    ...
    /// <inheritdoc />
    public async Task<OneBot10ActionTransportResult> SendAsync(
        string action,
        ...
        CancellationToken cancellationToken)
    {
        ...
        var requestParameters = OneBot10TransportPayload.Clone(parameters) as JsonObject ?? new JsonObject();
        var effectiveEcho = OneBot10TransportPayload.Clone(echo) ??
                            JsonValue.Create("onebotsdk.net:" + Guid.NewGuid().ToString("N"));
        if (effectiveEcho == null)
        {
            throw new OneBot10TransportException(
                OneBot10TransportError.ProtocolViolation,
        ...
        {
            ["action"] = action,
            ["params"] = OneBot10TransportPayload.Clone(requestParameters) as JsonObject ?? new JsonObject(),
            ["echo"] = OneBot10TransportPayload.Clone(effectiveEcho)
        };
        var rawRequestJson = OneBot10Json.Serialize(envelope);
        var echoKey = OneBot10EchoKey.Create(effectiveEcho);
        ...
        using (cancellationToken.Register(() => CancelPending(echoKey)))
        {
            try
            {
                ...
                return new OneBot10ActionTransportResult(
                    action,
                    requestParameters,
                    effectiveEcho,
                    rawRequestJson,
            ...
            catch (OneBot10TransportException exception)
            {
            ...
            {
                var failure = new OneBot10TransportException(
            ...
            try
            {
                while (_webSocket.State == WebSocketState.Open || _webSocket.State == WebSocketState.CloseSent)
                {
                    ...
                    if (result.MessageType == WebSocketMessageType.Close)
                    {
                        ...
                        FailAllPending(new OneBot10TransportException(
                            OneBot10TransportError.RemoteClosed,
                    ...
                    if (message.Length + result.Count > _options.MaxMessageBytes)
                    {
                        ...
                        throw OneBot10TransportPayload.TooLarge(_options.MaxMessageBytes);
                    }
    ...
    private void ProcessMessage(byte[] payload)
    {
        ...
        try
        {
            rawResponseJson = OneBot10TransportPayload.DecodeUtf8(payload);
            source = OneBot10TransportPayload.ParseObject(rawResponseJson);
        }
        catch (OneBot10TransportException exception)
        {
        ...
        if (source.TryGetPropertyValue("post_type", out postType) && postType != null)
        {
            try
            {
                _dispatcher.Dispatch(OneBot10EventParser.Parse(source));
            }
            catch (Exception exception)
            {
                RaiseFaulted(new OneBot10TransportException(
                    OneBot10TransportError.ProtocolViolation,
        ...
        if (source.TryGetPropertyValue("echo", out echo) && echo != null)
        {
            ...
            if (_pending.TryRemove(OneBot10EchoKey.Create(echo), out pending) && pending != null)
            {
    ...
    private async Task SendTextAsync(string text, CancellationToken cancellationToken)
    {
        ...
        catch (OneBot10TransportException)
        {
            throw;
        }
        catch (Exception exception)
    ...
    private void RaiseFaulted(OneBot10TransportException exception)
    {
    ...
    private static void AttachTrace(
        OneBot10TransportException exception,
        JsonObject requestParameters,
        JsonNode requestEcho,
        string rawRequestJson)
    {
        exception.RequestParameters = OneBot10TransportPayload.Clone(requestParameters) as JsonObject ?? new JsonObject();
        exception.RequestEcho = OneBot10TransportPayload.Clone(requestEcho);
    ...
    private void ThrowIfDisposed()
    {
        ...
        {
            throw new ObjectDisposedException(nameof(OneBot10WebSocketSession));
        }
    }
