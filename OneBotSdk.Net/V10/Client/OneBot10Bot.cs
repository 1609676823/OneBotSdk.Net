...
/// <summary>
/// Configures a OneBot 10 bot through separate action and event endpoints.
...
/// 通过相互独立的动作与事件终结点配置 OneBot 10 机器人。
/// 每个终结点都拥有自己的地址、访问令牌和传输专用设置。
/// 不存在共享令牌或跨终结点令牌回退。
/// </summary>
public sealed class OneBot10BotOptions
{
    /// <summary>
    ...
    /// </summary>
    /// <param name="actionEndpoint">The HTTP action address, token, and response settings. / HTTP 动作地址、令牌和响应设置。</param>
    /// <param name="eventEndpoint">The WebSocket event address, token, and session settings. / WebSocket 事件地址、令牌和会话设置。</param>
    /// <remarks>
    ...
    /// </remarks>
    public OneBot10BotOptions(
        OneBot10ActionEndpointOptions actionEndpoint,
        OneBot10EventEndpointOptions eventEndpoint)
    {
    ...
    /// <summary>
    /// Gets the HTTP action endpoint that owns its address, token, and response settings.
    /// 获取拥有自身地址、令牌和响应设置的 HTTP 动作终结点。
    /// </summary>
    public OneBot10ActionEndpointOptions ActionEndpoint { get; }

    /// <summary>
    /// Gets the WebSocket event endpoint that owns its address, token, and session settings.
    /// 获取拥有自身地址、令牌和会话设置的 WebSocket 事件终结点。
    /// </summary>
    public OneBot10EventEndpointOptions EventEndpoint { get; }

    internal OneBot10HttpActionTransportOptions CreateActionOptions()
    {
        return ActionEndpoint.CreateTransportOptions();
    }

    internal OneBot10ForwardWebSocketClientOptions CreateEventOptions()
    {
...
/// <summary>
/// Composes an HTTP action client and a forward WebSocket event listener without global state.
/// 在不使用全局状态的前提下组合 HTTP 动作客户端与正向 WebSocket 事件监听器。
/// </summary>
public sealed partial class OneBot10Bot : IDisposable
{
    ...
    public OneBot10Bot(OneBot10BotOptions options)
        ...
        Func<CancellationToken, Task>? startEventConnector)
    {
        ...
        Events = dispatcher ?? new OneBot10EventDispatcher();
        ActionTransport = new OneBot10HttpActionTransport(actionOptions, httpClient);
        EventTransport = new OneBot10ForwardWebSocketClient(eventOptions, Events);
        Actions = new OneBot10Client(ActionTransport);
    ...
    /// <exception cref="OneBot10BotStartException">The <c>get_login_info</c> action returned a failed response. / <c>get_login_info</c> 动作返回失败响应。</exception>
    /// <exception cref="OneBot10TransportException">An HTTP or WebSocket transport failed. / HTTP 或 WebSocket 传输失败。</exception>
    ...
    public OneBot10Response<OneBot10LoginInfoData> Start(CancellationToken cancellationToken = default)
    {
    ...
    /// <exception cref="ObjectDisposedException">The bot has been disposed. / 机器人已经释放。</exception>
    /// <exception cref="InvalidOperationException">The event endpoint is already connected. / 事件终结点已经连接。</exception>
    public async Task<OneBot10Response<OneBot10LoginInfoData>> StartAsync(CancellationToken cancellationToken = default)
    {
        ThrowIfDisposed();
        ...
        try
        {
            ...
            {
                throw new OneBot10BotStartException(loginInfoResponse);
            }
    ...
    private void ThrowIfDisposed()
    {
        ...
        {
            throw new ObjectDisposedException(nameof(OneBot10Bot));
        }
    }
}
