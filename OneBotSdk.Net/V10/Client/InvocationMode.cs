...
/// <summary>
/// Selects the standard OneBot 10 action invocation suffix.
/// 选择标准 OneBot 10 动作调用后缀。
/// </summary>
...
internal static class OneBot10ActionName
{
    internal static string ApplyInvocationMode(string action, InvocationMode mode)
    {
        ...
        // Invocation modes are mutually exclusive because OneBot 10 defines no combined suffix form.
        // 调用模式互斥，因为 OneBot 10 未定义组合后缀形式。
        switch (mode)
        {
            ...
            default:
                throw new ArgumentOutOfRangeException(nameof(mode), mode, "Unknown OneBot 10 invocation mode.");
        }
    }
}
